...
export function UseItemForm({ items }: { items: ItemForConsumption[] }) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      ...
      if (result.success) {
        setSuccessMsg(
          `Recorded ${consumptionQty} ${selectedItem?.unit} of ${selectedItem?.name}. Balance: ${balanceAfter.toFixed(3)} ${selectedItem?.unit}`
        );
        setSelectedItemId('');
        setQty('');
        setNotes('');
    ...
    return (
      <Card className="border-dashed">
        <CardContent className="py-14 text-center">
          ...
          <p className="font-medium text-muted-foreground">No items in stock</p>
          <p className="text-sm text-muted-foreground mt-1 max-w-xs mx-auto">
            Purchase items first via the Shopping page, then return here to record consumption.
          </p>
        </CardContent>
      </Card>
    );
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5 max-w-lg">
      ...
      <div className="space-y-1.5">
        ...
        >
          <SelectTrigger className="h-10">
            <SelectValue placeholder="Choose an item from your stock..." />
          </SelectTrigger>
          <SelectContent>
            {Array.from(itemsByCategory.entries()).map(([cat, catItems]) => (
              <SelectGroup key={cat}>
                <SelectLabel className="text-xs font-bold text-muted-foreground uppercase tracking-wide">
                  {cat}
                </SelectLabel>
                {catItems.map(item => (
                  <SelectItem key={item.id} value={String(item.id)}>
                    ...
                    <span className="text-muted-foreground ml-1.5 text-xs">
                      — {parseFloat(item.stockQty)} {item.unit} available
                    </span>
                  </SelectItem>
                ))}
              </SelectGroup>
            ...
            <div className="flex items-center justify-between gap-4">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">
                  ₹{parseFloat(selectedItem.price).toLocaleString('en-IN')} per {selectedItem.unit}
                </p>
              </div>
              <div className="text-right shrink-0">
                {stockQty === 0 ? (
                  ...
                  <Badge variant="destructive">Critical — {stockQty} {selectedItem.unit}</Badge>
                ) : stockQty <= 20 ? (
                  <Badge variant="warning">Low — {stockQty} {selectedItem.unit}</Badge>
                ) : (
                  <Badge variant="success">In Stock — {stockQty} {selectedItem.unit}</Badge>
                )}
                <p className="text-xs text-muted-foreground mt-1">
                  Value: ₹{(stockQty * parseFloat(selectedItem.price)).toLocaleString('en-IN')}
                </p>
              </div>
            </div>
          </CardContent>
      ...
      {/* Quantity input */}
      <div className="space-y-1.5">
        <Label htmlFor="consQty">
          Consumption Quantity{selectedItem ? ` (${selectedItem.unit})` : ''} *
        </Label>
        <Input
          id="consQty"
          type="number"
          ...
          placeholder={selectedItem ? `Max: ${stockQty} ${selectedItem.unit}` : 'Select an item first'}
      ...
      {selectedItem && qty && consumptionQty > 0 && (
        <div
          className={`rounded-lg border px-4 py-3 text-sm flex items-center gap-2.5 ${
            balanceAfter < 0
        ...
        >
          ...
          <span>
            Balance after use:{' '}
            <strong>
              {Math.max(0, balanceAfter).toFixed(3)} {selectedItem.unit}
            </strong>{' '}
            (₹
            {(Math.max(0, balanceAfter) * parseFloat(selectedItem.price)).toLocaleString('en-IN')}
            )
          </span>
        </div>
      )}
      ...
      {/* Notes */}
      <div className="space-y-1.5">
        <Label htmlFor="consNotes">Notes (optional)</Label>
        <Textarea
          id="consNotes"
          value={notes}
          onChange={e => setNotes(e.target.value)}
          placeholder="e.g. Used for cooking, daily consumption..."
          rows={2}
        />
      </div>
