...
function stockBadge(qty: number, unit: string) {
  if (qty === 0) return <Badge variant="destructive">Out of Stock</Badge>;
  if (qty <= 5) return <Badge variant="destructive">Critical</Badge>;
  if (qty <= 20) return <Badge variant="warning">Low</Badge>;
...
export function ConsumptionBalanceReport({ rows }: { rows: BalanceReportRow[] }) {
  ...
  const totalConsumedQty = filtered.length; // item count
  ...
  if (rows.length === 0) {
    return (
      <div className="py-16 text-center text-muted-foreground">
        ...
        <p className="font-medium">No balance data yet</p>
        <p className="text-sm mt-1">Purchase items and record consumption to see the balance report.</p>
      </div>
    );
  }
        ...
        <Card className="border-blue-100 bg-blue-50/50">
          <CardHeader className="pb-1 pt-3 px-4">
            <CardTitle className="text-xs font-medium text-blue-700 flex items-center gap-1.5">
              <ShoppingCart className="h-3.5 w-3.5" /> Total Purchased
            </CardTitle>
          </CardHeader>
        ...
        <Card className="border-orange-100 bg-orange-50/50">
          <CardHeader className="pb-1 pt-3 px-4">
            <CardTitle className="text-xs font-medium text-orange-700 flex items-center gap-1.5">
              <TrendingDown className="h-3.5 w-3.5" /> Consumed Value
            </CardTitle>
          </CardHeader>
          <CardContent className="pb-3 px-4">
        ...
        <Card className="border-emerald-100 bg-emerald-50/50">
          <CardHeader className="pb-1 pt-3 px-4">
            <CardTitle className="text-xs font-medium text-emerald-700 flex items-center gap-1.5">
              <IndianRupee className="h-3.5 w-3.5" /> Balance Value
            </CardTitle>
          </CardHeader>
          <CardContent className="pb-3 px-4">
        ...
        <Card className="border-purple-100 bg-purple-50/50">
          <CardHeader className="pb-1 pt-3 px-4">
            <CardTitle className="text-xs font-medium text-purple-700 flex items-center gap-1.5">
              <Scale className="h-3.5 w-3.5" /> Items Tracked
            </CardTitle>
          </CardHeader>
          <CardContent className="pb-3 px-4">
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-muted/30">
              ...
              <TableHead className="text-xs text-right">Purchased Qty</TableHead>
              <TableHead className="text-xs text-right">Purchased Amt</TableHead>
              <TableHead className="text-xs text-right">Consumed Qty</TableHead>
              <TableHead className="text-xs text-right">Balance Qty</TableHead>
              <TableHead className="text-xs text-right">Balance Value</TableHead>
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map(row => (
                <TableRow key={row.id} className="hover:bg-muted/10 text-sm">
                  ...
                  <TableCell>{stockBadge(row.balanceQty, row.unit)}</TableCell>
                </TableRow>
              ))
            )}
          </TableBody>
